...
from ldm.modules.attention import SpatialTransformer, SpatialMutualTransformer
    ...
    def forward(self, x1, x2, emb1, emb2, context1=None, context2=None):
        for layers in self:
            ...
            if isinstance(layer, TimestepBlock):
                ...
                layer = layers[1]
                x2 = layer(x2, emb2)
            elif isinstance(layer, SpatialMutualTransformer):
                assert len(layers) == 1, "There should be only 1 SpatialMutualTransformer Layer"
                x1, x2 = layer(x1, x2, context1, context2)
            else:
                assert len(layers) == 1, "There should be only 1 MutualAttentionBlockLayer"
                x1, x2 = layer(x1, x2)
        return x1, x2
    ...
    def forward(self, x1, x2, emb1, emb2, context1=None, context2=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                ...
                emb = th.cat([emb1, emb2], dim=1)
                x = layer(x, emb)
                x1, x2 = th.chunk(x, chunks=2, dim=1)
            elif isinstance(layer, SpatialMutualTransformer):
                x1, x2 = layer(x1, x2, context1, context2)
            else:
                x1, x2 = layer(x1, x2)
...
class MutualQKVAttentionLegacy(nn.Module):
    """
    A module which performs QKV attention for mutual attention
    """
    ...
    def forward(self, qkv1, qkv2):
        assert qkv1.shape == qkv2.shape, 'QKV1 and QKV2 should have the same dimension'
        bs, width, length = qkv1.shape
        assert width % (3 * self.n_heads) == 0
        ch = width // (3 * self.n_heads)
        q1, k1, v1 = qkv1.reshape(bs * self.n_heads, ch * 3, length).split(ch, dim=1)
        q2, k2, v2 = qkv2.reshape(bs * self.n_heads, ch * 3, length).split(ch, dim=1)
        scale = 1 / math.sqrt(math.sqrt(ch))
        weight1 = th.einsum(
            "bct,bcs->bts", q1 * scale, th.cat([k1, k2], dim=-1) * scale
        )
        weight1 = th.softmax(weight1.float(), dim=-1).type(weight1.dtype)
        a1 = th.einsum("bts,bcs->bct", weight1, th.cat([v1, v2], dim=-1))

        weight2 = th.einsum(
            "bct,bcs->bts", q2 * scale, k2 * scale
        )
        weight2 = th.softmax(weight2.float(), dim=-1).type(weight2.dtype)
        a2 = th.einsum("bts,bcs->bct", weight2, v2)
...
class MutualQKVAttention(nn.Module):
    def __init__(self, n_heads):
        super().__init__()
        self.n_heads = n_heads
    
    def forward(self, qkv1, qkv2):
        assert qkv1.shape == qkv2.shape
        bs, width, length = qkv1.shape
        assert width % (3 * self.n_heads) == 0
        ch = width // (3 * self.n_heads)
        ...
        scale = 1 / math.sqrt(math.sqrt(ch))
        k = th.cat([k1, k2], dim=-1)
        v = th.cat([v1, v2], dim=-1)
        weight1 = th.einsum(
            "bct,bcs->bts",
            (q1 * scale).view(bs * self.n_heads, ch, length),
            (k * scale).view(bs * self.n_heads, ch, length),
        )
        weight1 = th.softmax(weight1.float(), dim=-1).type(weight1.dtype)
        a1 = th.einsum("bts,bcs->bct", weight1, v.reshape(bs * self.n_heads, ch, length))

        weight2 = th.einsum(
            "bct,bcs->bts",
            (q2 * scale).view(bs * self.n_heads, ch, length),
            (k2 * scale).view(bs * self.n_heads, ch, length),
        )
        weight2 = th.softmax(weight2.float(), dim=-1).type(weight2.dtype)
        a2 = th.einsum("bts,bcs->bct", weight2, v2.reshape(bs * self.n_heads, ch, length))
...
class MutualSourceUNetModel(nn.Module):
    def __init__(
        ...
        use_spatial_transformer=False,    # custom transformer support
        transformer_depth=1,              # custom transformer support
        context1_dim=None,                 # custom transformer support
        context2_dim=None,
        n_embed=None,                     # custom support for prediction of discrete ids into codebook of first stage vq model
        legacy=True,
        disable_self_attentions=None,
    ...
    def forward(self, x_t, x_r, timesteps=None, context_t=None, context_r=None, y=None, **kwargs):
        """
        Apply the model to an input pair
        :param x_t: target x dimension [N x C x ...]
        :param x_r: reference x dimension [N x C x ...]
        :param timesteps: a 1-D batch of timesteps
        :param context_t: conditioning plugin of target crossattn
        :param context_r: conditioning plugin of reference via crossattn
        """

        hs_t, hs_r = [], []
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        emb_t = self.time_embed_t(t_emb)
        emb_r = self.time_embed_r(t_emb)

        h_t, h_r = x_t.type(self.dtype), x_r.type(self.dtype)

        for module in self.input_blocks:
            h_t, h_r = module(h_t, h_r, emb_t, emb_r, context_t, context_r)
            hs_t.append(h_t)
            hs_r.append(h_r)
        h_t, h_r = self.middle_block(h_t, h_r, emb_t, emb_r, context_t, context_r)
        for module in self.output_blocks:
            h_t = th.cat([h_t, hs_t.pop()], dim=1)
            h_r = th.cat([h_r, hs_r.pop()], dim=1)
            h_t, h_r = module(h_t, h_r, emb_t, emb_r, context_t, context_r)
        
        h_t, h_r = h_t.type(self.dtype), h_r.type(self.dtype)
        if self.predict_codebook_ids:
...
class MutualAttentionUNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.input_blocks = nn.ModuleList(
            [
                MutualTimestepEmbedSequential(
                    conv_nd(dims, in_channels * 2, model_channels * 2, 3, padding=1)
                )
            ]
        )
        self._feature_size = model_channels
            ...
            for nr in range(self.num_res_blocks[level]):
                layers = [
                    ResBlock(
                        ...
                        time_embed_dim * 2,
                        dropout,
                        out_channels=mult * model_channels * 2,
                        dims=dims,
                        use_checkpoint=use_checkpoint,
                        use_scale_shift_norm=use_scale_shift_norm,
                    )
            ...
            for i in range(self.num_res_blocks[level] + 1):
                ...
                layers = [
                    ResBlock(
                        ...
                        time_embed_dim * 2,
                        dropout,
                        out_channels=model_channels * mult * 2,
                        dims=dims,
                        use_checkpoint=use_checkpoint,
                        use_scale_shift_norm=use_scale_shift_norm,
                    )
    ...
    def forward(self, x_t, x_r, timesteps=None, context_t=None, context_r=None, y=None,**kwargs):
        ...
        h_t = x_t.type(self.dtype)
        h_r = x_r.type(self.dtype)
        for module in self.input_blocks:
            h_t, h_r = module(h_t, h_r, emb_t, emb_r, context_t, context_r)
            hs_t.append(h_t)
            hs_r.append(h_r)
