...
class MutualDiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        super().__init__()
        self.sequential_cross_attn = diff_model_config.pop("sequential_crossattn", False)
        self.diffusion_model = instantiate_from_config(diff_model_config)
    ...
    def forward(self, x_t, x_r, t, c_t_concat: list = None, c_t_crossattn: list = None, c_t_adm=None, c_r_concat: list = None, c_r_crossattn: list = None, c_r_adm=None):
        if self.conditioning_key is None:
            ...
            out = self.diffusion_model(xc_t, xc_r, t)
        elif self.conditioning_key == 'crossattn':
            if not self.sequential_cross_attn:
                if c_t_crossattn[0] is not None:
                    cc_t = torch.cat(c_t_crossattn, 1)
                else:
                    cc_t = c_t_crossattn[0]
                if c_r_crossattn[0] is not None:
                    cc_r = torch.cat(c_r_crossattn, 1)
                else:
                    cc_r = c_r_crossattn[0]
            else:
                cc_t = c_t_crossattn
                cc_r = c_r_crossattn
            out = self.diffusion_model(x_t, x_r, t, context_t=cc_t, context_r=cc_r)
        elif self.conditioning_key == 'hybrid':
            xc_t = torch.cat([x_t] + c_t_concat, dim=1)
            xc_r = torch.cat([x_r] + c_r_concat, dim=1)
            cc_t = torch.cat(c_t_crossattn, 1)
            cc_r = torch.cat(c_r_crossattn, 1)
            out = self.diffusion_model(xc_t, xc_r, t, context_t=cc_t, context_r=cc_r)
        elif self.conditioning_key == 'hybrid-adm':
            assert c_t_adm is not None and c_r_adm is not None
            xc_t = torch.cat([x_t] + c_t_concat, dim=1)
            xc_r = torch.cat([x_r] + c_r_concat, dim=1)
            cc_t = torch.cat(c_t_crossattn, 1)
            cc_r = torch.cat(c_r_crossattn, 1)
            out = self.diffusion_model(xc_t, xc_r, t, context_t=cc_t, context_r=cc_r, y=c_t_adm)
        elif self.conditioning_key == 'crossattn-adm':
            assert c_r_adm is not None and c_t_adm is not None
            cc_t = torch.cat(c_t_crossattn, 1)
            cc_r = torch.cat(c_r_crossattn, 1)
            out = self.diffusion_model(x_t, x_r, t, context_t=cc_t, context_r=cc_r, y=c_t_adm)
        elif self.conditioning_key == 'adm':
            cc_t = c_t_crossattn[0]
            cc_r = c_r_crossattn[1]
            out = self.diffusion_model(x_t, x_r, t, y=cc_t)
        else:
            raise NotImplementedError()
