...
if __name__ == "__main__":
    ...
    mutual_model = create_model('./modesl/mutual_ldm_v15.yaml')
    mutual_model.load_state_dict(load_state_dict('./models/mutual_epoch10.ckpt', location='cuda'))
    ...
    for i in range(len(dataset)):
        ...
        if i == 0:
            x_sample = single_model.sample(cond=cond, batch_size=1, return_intermediates=False, verbose=False, shape=shape)
            x_sample = single_model.decode_first_stage(x_sample)
            reference = x_sample
        else:
            x_sample = mutual_model.sample(cond=cond, reference=reference, reference_location=reference_location, 
                                           batch_size=1, return_intermediates=False, verbose=False, shape=shape)
